...
export function InventarioPage({ complexId }: InventarioPageProps) {
  return (
...
function ZonesSection({ complexId }: { complexId: Id<'complexes'> }) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Zonas sociales</CardTitle>
      </CardHeader>
    ...
    <div className="flex items-center gap-2">
      ...
      <Switch
        ...
        onCheckedChange={(checked) => {
          if (checked) onChange({ start: 420, end: 1320 })
          else onChange(null)
        }}
      />
      {isClosed ? (
...
function BlocksSection({ complexId }: { complexId: Id<'complexes'> }) {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Bloqueos de fechas</CardTitle>
      </CardHeader>
                ...
                >
                  <div className="flex flex-col gap-0.5">
                    <span className="text-sm">
                      {block.date} &middot;{' '}
                      {zone ? zone.name : 'Todas las zonas'}
                    </span>
                    {block.reason && (
